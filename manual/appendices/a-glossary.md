# Appendix A · Glossary 📖

> ⏱️ 25 min read · 🎯 Everyone (keep it open in a tab!) · 🧰 Needs: nothing

**Every piece of AI jargon in this manual, in plain English, with a 🧸 ELI5 for each one.** Jump to a letter with the
sidebar or the cards below. Tip: on the website, **acronyms anywhere in the manual show their ELI5 when you hover over
them** (or tap them on a phone). That magic comes from this page. ✨

<details class="eli5" open>
<summary>🧸 ELI5: This page in 30 seconds</summary>

This is the dictionary at the back of the book. Every tricky AI word is here, first with a grown-up explanation and then with a
super-simple one, so you'll never feel lost. 📚🙂

</details>

<!-- in-this-chapter -->

## 🅰️ A

| Term | Meaning | 🧸 ELI5 |
|---|---|---|
| **A2A (Agent2Agent)** | An open protocol for AI agents built by different companies or frameworks to talk to and delegate to each other. | A way for robot helpers from different companies to talk to each other. |
| **AAC** | Augmentative and alternative communication: tools that help people who can't rely on speech to communicate, increasingly with AI voices. | Tools that help people talk when speaking is hard. |
| **Adaptive thinking / effort** | Settings that let a model decide how much to "think" before answering, or let you dial reasoning depth up or down. | Telling the AI how hard to think: quick answer or deep thought. |
| **Agent** | An AI system that works in a loop (think → use a tool → look at the result → repeat) until a goal is done. | An AI with hands that can do jobs, not just talk. |
| **Agent loop** | The think/act/observe cycle an agent repeats until it finishes or hits a limit. | Think, do, look, repeat, until the job is done. |
| **Agent SDK (Claude Agent SDK)** | A library that gives your own apps the same agent harness that powers Claude Code. | Claude Code's engine, as LEGO blocks for your own apps. |
| **Agentic RAG** | RAG where an agent decides what to search, searches several times and checks results. | A librarian robot that keeps searching until it's sure. |
| **AGENTS.md / CLAUDE.md** | Markdown files in a project that coding agents read automatically as standing instructions. | A sticky note of house rules the AI reads first. |
| **AGI** | Artificial general intelligence: a hypothetical AI as capable as humans across most tasks. Definitions vary and are debated. | A future AI that could do almost anything a person can. |
| **Alt text** | A short text description of an image for people using screen readers. | Words that describe a picture for people who can't see it. |
| **ANN** | Approximate nearest neighbor search: fast, nearly-exact search for similar vectors. | A shortcut for finding the closest matches super fast. |
| **API** | A way for programs to talk to each other. AI APIs let your code send prompts and get answers, usually billed per token. | The kitchen's back door your programs use to order from the AI. |
| **API key** | A secret string that identifies you (and your bill) when calling an API. | A secret membership card for using an AI service. |
| **Artifact** | A generated, often interactive piece of content (an app, doc or chart) shown alongside a Claude chat. | A little app or picture the AI makes right in the chat. |
| **ASR** | Automatic speech recognition, also called speech-to-text. | Turning spoken words into written words. |
| **ATS** | Applicant tracking system: software employers use to collect and filter job applications. | The computer that reads job applications first. |
| **Audio Overview** | A Gemini Notebook (NotebookLM) feature that turns your sources into a two-host podcast. | Your documents turned into a radio show. |
| **Autonomy levels** | How much an AI does on its own: from suggestions, to drafts, to acting with approval, to fully automatic. | How much the robot is allowed to do without asking. |

## 🅱️ B

| Term | Meaning | 🧸 ELI5 |
|---|---|---|
| **Background agent** | A coding agent that works in the cloud on its own branch while you do other things. | A robot helper that works while you're away. |
| **Barge-in** | Interrupting a voice agent while it's talking. Good agents stop and listen. | Talking over the robot, and it stops to listen. |
| **Batch API** | Sending many non-urgent requests at once for a big discount, with results later. | Ordering in bulk and picking up later, for cheaper. |
| **Benchmark** | A standard test set used to compare models. | A school test that all AIs take so we can compare them. |
| **Bias (AI)** | Unfair patterns a model learned from data, which can affect outputs about people. | When AI treats some people unfairly because of what it learned. |
| **BM25** | A classic keyword-ranking algorithm, often combined with vector search in hybrid search. | Old-school search that matches exact words. |
| **Body doubling** | Working alongside someone (or a voice AI) to stay focused. | Having a buddy nearby so you keep working. |

## ©️ C

| Term | Meaning | 🧸 ELI5 |
|---|---|---|
| **C2PA / content credentials** | A standard for attaching tamper-evident "how this was made" info to images and videos. | A label that says how a picture was made. |
| **Caching (prompt caching)** | Reusing processed prompt prefixes to make repeated calls cheaper and faster. | Remembering the long instructions so you don't pay for them again. |
| **Chain / workflow** | A fixed sequence of steps, some of which use AI. | A recipe: do step 1, then 2, then 3. |
| **Checkpoint** | A saved state you can roll back to (in Claude Code, or a build-along's "it works so far" point). | A save point in a video game. |
| **Chunking** | Splitting documents into passages before embedding them for RAG. | Cutting a big book into index cards. |
| **CLI** | Command-line interface: a program you use by typing commands in a terminal. | Talking to your computer by typing commands. |
| **Code execution** | A sandbox where the AI writes and runs code (often Python) to analyze data or check work. | A safe playroom where the AI can run little programs. |
| **Coding agent** | An AI that reads, writes, runs and fixes code on its own (Claude Code, Codex, Copilot agent…). | A robot programmer. |
| **Commit (Git)** | A saved snapshot of your project with a message. | A save point for your project. |
| **Compaction** | Summarizing older parts of a long conversation to free up context. | Squishing old notes into a short summary to make room. |
| **Computer use** | AI controlling a mouse, keyboard and screen (or a browser) like a human would. | AI that clicks and types on a computer by itself. |
| **Connector** | A ready-made integration in an AI app (often MCP under the hood) that links it to a service like Gmail. | A plug that connects the AI to one of your apps. |
| **Context engineering** | Deliberately choosing what goes into the model's context: instructions, examples, documents, tools. | Packing the AI's backpack with exactly what it needs. |
| **Context window** | How much text (in tokens) a model can consider at once: its working memory. | The size of the AI's desk. |
| **Cosine similarity** | A measure of how closely two vectors point in the same direction. It's how RAG finds similar text. | Checking if two arrows point the same way. |
| **CRM** | Customer relationship management: software (or a spreadsheet) for tracking contacts and deals. | An address book for customers. |
| **CRUD** | Create, read, update, delete: the four basic things apps do with data. | Make it, see it, change it, throw it away. |
| **Custom connector** | A remote MCP server you add to an AI app by pasting its URL. | Your own plug, added by pasting a link. |

## 🇩 D

| Term | Meaning | 🧸 ELI5 |
|---|---|---|
| **Deep research** | An assistant mode that runs a multi-step, multi-source investigation and writes a cited report. | The AI goes to the library for an hour and writes you a report. |
| **Deepfake** | Realistic fake video, audio or images of real people made with AI. | A fake video that looks real. Be careful! |
| **Diff** | The list of changes between two versions of files. | A "spot the difference" of your project. |
| **Distillation** | Training a smaller model on a bigger model's outputs. | A big teacher robot teaching a small student robot. |
| **Docker / container** | A packaged, isolated app that runs the same everywhere. | A lunchbox with everything a program needs inside. |
| **DuckDB** | A tiny, fast database that runs SQL directly on CSV and Parquet files. | A pocket calculator for big tables. |

## 🇪 E

| Term | Meaning | 🧸 ELI5 |
|---|---|---|
| **Edge function** | A small piece of server code that runs close to users, on demand. | A tiny helper that wakes up only when needed. |
| **Elicitation** | An MCP feature where a server asks *you* a question mid-task. | The plug-in asking you a quick question. |
| **Embedding** | A list of numbers that represents the meaning of text (or images). Similar meanings get similar numbers. | A meaning-address on a giant map. |
| **Endpointing** | Deciding when a speaker has finished their turn, in voice agents. | Knowing when you've stopped talking. |
| **Environment variable** | A named setting (like an API key) given to a program when it runs, instead of being written in the code. | A secret note the program reads when it starts. |
| **Eval (evaluation)** | A repeatable test set for measuring AI quality on your tasks. | A quiz you give the AI to see how well it does. |

## 🇫 F

| Term | Meaning | 🧸 ELI5 |
|---|---|---|
| **Few-shot prompting** | Including a few examples in the prompt to show the pattern you want. | Showing the AI a few examples first. |
| **Fine-tuning** | Further training a model on your examples. Great for style and format, not for facts. | Cooking lessons in your style for a great chef. |
| **Frontier model** | The most capable models available at a given time. | The smartest AIs right now. |
| **Function calling / tool use** | The model outputting a structured request to run a tool, which your software executes. | The AI asking your program to press a button for it. |

## 🇬 G

| Term | Meaning | 🧸 ELI5 |
|---|---|---|
| **GDPR** | The European Union's data-protection law, giving people rights over their personal data. | Europe's rule book for keeping personal information safe. |
| **Gemini Notebook (formerly NotebookLM)** | Google's source-grounded research notebook with Audio and Video Overviews, renamed in July 2026. | A study buddy that only reads the books you give it. |
| **GGUF** | A file format for quantized local models, used by llama.cpp, Ollama and LM Studio. | The file type for home AI brains. |
| **Git** | A version-control system that saves the history of your project. | A time machine for your files. |
| **GitHub Actions** | Automations that run on GitHub when things happen (a push, a schedule, a comment). | Little robots that work on your project when you save it. |
| **GPU** | Graphics processing unit: a chip that's excellent at the math AI needs. | The super-fast math engine inside gaming computers. |
| **Grounding** | Making a model answer from specific sources (documents, search results) and cite them. | Only answering from the books on the table. |
| **GraphRAG** | RAG that builds a knowledge graph of entities and relationships to answer connection questions. | A map of who's connected to what. |
| **Guardrails** | Rules, checks and limits that keep AI systems safe and on-task. | Bumpers in bowling, so the ball stays in the lane. |

## 🇭 H

| Term | Meaning | 🧸 ELI5 |
|---|---|---|
| **Hallucination** | The model confidently stating something false. Grounding and verification reduce it. | The AI making something up and sounding sure. |
| **Handoff** | One agent passing a conversation or task to another (or to a human). | Passing the baton in a relay race. |
| **Headless mode** | Running an agent (like Claude Code) from scripts or CI without an interactive chat. | The robot working without anyone watching. |
| **HNSW** | A graph-based index that makes vector search fast. | A highway map for finding similar things quickly. |
| **Hook** | An automatic script that runs on agent events (e.g. "after each file edit, run the formatter"). | A rule that always happens automatically. |
| **Host / client / server (MCP)** | Host = the AI app. Client = its connection manager. Server = the program exposing tools and data. | The house, the doorbell and the visitor with gifts. |
| **Human-in-the-loop** | A step where a person approves or edits before the AI's output is used. | A grown-up checks before anything important happens. |
| **Hybrid search** | Combining keyword and vector search for better retrieval. | Searching by exact words and by meaning at the same time. |

## 🇮 I

| Term | Meaning | 🧸 ELI5 |
|---|---|---|
| **IDE** | Integrated development environment: a code editor with extras (VS Code, Cursor, JetBrains). | A word processor for writing programs. |
| **Image-to-video** | Animating a still image with a video model. | Making a picture move. |
| **Inference** | Running a model to get output (as opposed to training it). | The AI thinking up an answer. |
| **Inpainting / outpainting** | Editing part of an image, or extending it beyond its edges. | Fixing a patch of a picture, or making it bigger. |

## 🇯 J

| Term | Meaning | 🧸 ELI5 |
|---|---|---|
| **JSON** | The universal text format for structured data (`{"key": "value"}`). | A neat, labeled list computers love. |
| **JSON Schema** | A description of what valid JSON looks like. Tools use it to define their parameters. | The shape a form must have. |
| **Jupyter notebook** | A document mixing notes, code and charts, run step by step. | A science notebook that can run code. |

## 🇰 K

| Term | Meaning | 🧸 ELI5 |
|---|---|---|
| **Knowledge base** | A collection of documents an AI can search to answer questions. | The AI's personal library. |
| **Knowledge cutoff** | The date a model's training data ends. It doesn't know later events unless given tools or documents. | The last day the AI read the news. |

## 🇱 L

| Term | Meaning | 🧸 ELI5 |
|---|---|---|
| **Latency** | The delay before a response arrives. Crucial for voice. | How long you wait for an answer. |
| **Lethal trifecta** | Private data + untrusted content + a way to send data out, in one AI setup: a recipe for prompt-injection harm. | Three things that are risky together, like matches, paper and wind. |
| **LLM** | Large language model: an AI trained on huge amounts of text to predict and generate language. | A giant autocomplete that read the whole library. |
| **LLM-as-judge** | Using a model to grade other models' outputs against a rubric. | An AI teacher grading other AIs' homework. |
| **Local model** | A model running on your own device instead of the cloud. | A pet robot at home instead of phoning one. |
| **LoRA** | Low-rank adaptation: a small add-on trained to teach a model a style, character or skill. | A sticky note that changes how the AI behaves. |

## Ⓜ️ M

| Term | Meaning | 🧸 ELI5 |
|---|---|---|
| **MCP** | Model Context Protocol: the open standard for connecting AI apps to tools and data. | USB ports for AI. |
| **MCP Apps** | An MCP extension that lets servers show interactive UI inside the chat. | Plug-ins that can show buttons and pictures in chat. |
| **MCP Inspector** | A web tool for testing MCP servers by hand: list tools, call them, read raw messages. | A test window for pressing a plug-in's buttons. |
| **MCP Registry** | The official public list of MCP servers that clients and catalogs can discover. | The app store list for AI plug-ins. |
| **Memory (AI)** | Facts and preferences an assistant keeps across conversations. | The AI's notebook about you. |
| **Mixture of experts** | A model design where only some "expert" sub-networks run per token: big-model smarts, small-model speed. Often written "MoE". | A team of specialists where only a few work at a time. |
| **MLX** | Apple's machine-learning framework, fast on Apple Silicon. | Apple's engine for running AI on Macs. |
| **Model routing** | Sending easy tasks to cheap models and hard ones to frontier models. | Asking the junior helper first, the expert only when needed. |
| **Multimodal** | Handling more than text: images, audio, video. | AI that can see and hear, not just read. |

## 🇳 N

| Term | Meaning | 🧸 ELI5 |
|---|---|---|
| **n8n** | An open-source, self-hostable visual automation platform with strong AI features. | LEGO blocks for building robots that do chores. |
| **Node (n8n / Make)** | One step in a visual workflow. | One LEGO block in the robot. |
| **NPU** | Neural processing unit: a chip in newer laptops and phones for efficient AI tasks. | A small AI engine in your phone. |

## 🇴 O

| Term | Meaning | 🧸 ELI5 |
|---|---|---|
| **OAuth** | The "Log in with…" flow that grants an app limited access without sharing your password. | A visitor pass instead of your house key. |
| **OCR** | Optical character recognition: turning images of text into actual text. | Reading words out of a photo. |
| **Ollama** | A popular tool for running open models locally with one command. | The easiest way to get an AI brain on your laptop. |
| **Open-weight model** | A model whose weights you can download and run yourself (Gemma, Qwen, Llama, gpt-oss…). | An AI brain anyone can take home. |
| **OpenAI-compatible API** | An API that copies OpenAI's format, so tools built for one work with many (including local models). | A plug shape lots of devices fit. |
| **Orchestrator** | In multi-agent systems, the lead agent that plans and delegates to worker agents. | The boss robot handing out jobs. |

## 🇵 P

| Term | Meaning | 🧸 ELI5 |
|---|---|---|
| **PARA** | A note-organizing method: Projects, Areas, Resources, Archive. | Four drawers for all your notes. |
| **Parameters (model)** | The learned numbers inside a model. More usually means more capable (and hungrier). | The knobs inside the AI's brain. |
| **pause_turn** | An API stop reason meaning a long server-side tool turn paused, and you should continue the conversation. | The AI saying "one sec, let me keep going." |
| **Plan mode** | A Claude Code mode where it researches and proposes a plan without changing anything. | Drawing the map before starting the trip. |
| **Plugin** | A bundle of skills, commands, subagents, hooks and MCP servers you install at once. | A gift box of new tricks. |
| **Progressive disclosure** | Loading only a short summary (like a skill's description) until the full content is needed. | Reading book titles first, and the whole book only when needed. |
| **Prompt** | The instructions and content you give a model. | What you tell the AI to do. |
| **Prompt injection** | Malicious instructions hidden in content the AI reads. | Sneaky secret orders hidden in a web page. |
| **Prompt template (MCP prompt)** | A reusable prompt a server offers, often shown as a slash command. | A fill-in-the-blanks message. |
| **Pull request (PR)** | A proposed set of changes on GitHub for review before merging. | Handing in homework for checking. |

## 🇶 Q

| Term | Meaning | 🧸 ELI5 |
|---|---|---|
| **Quantization** | Compressing a model (e.g. to 4-bit) so it fits on smaller hardware. | Squishing a big brain to fit a small backpack. |

## 🇷 R

| Term | Meaning | 🧸 ELI5 |
|---|---|---|
| **RAG** | Retrieval-augmented generation: fetch relevant snippets, then have the model answer using them. | Giving the AI a library card for your documents. |
| **Rate limit** | A cap on how many requests you can make in a time window (HTTP 429 when exceeded). | "Slow down, one at a time!" |
| **Realtime / speech-to-speech model** | A model that listens and speaks directly, for low-latency voice agents. | One brain that hears and talks at once. |
| **Reasoning model** | A model that "thinks" step by step before answering, improving hard problems. | An AI that shows its working on a math problem. |
| **Remote MCP server** | An MCP server reached over the internet (Streamable HTTP) instead of launched locally. | A plug-in that lives on the internet. |
| **Repository (repo)** | A project folder tracked by Git, often hosted on GitHub. | A project folder with a time machine inside. |
| **Reranking** | A second pass that re-orders retrieved results by relevance. | A second expert sorting the best cards to the top. |
| **Resource (MCP)** | Data a server exposes for the app or user to attach as context. | A document the plug-in can hand over. |
| **REST API** | The common style of web API using HTTP methods (GET, POST…) on URLs. | The usual way websites talk to each other. |
| **RLHF** | Reinforcement learning from human feedback: training models using people's ratings of outputs. | Teaching the AI with thumbs up and thumbs down. |
| **RLS** | Row-level security: database rules so each user only sees their own rows (e.g. in Supabase). | A lock so you only see your own stuff. |
| **Rubric** | A list of specific criteria for scoring outputs. | A checklist for grading. |

## 🇸 S

| Term | Meaning | 🧸 ELI5 |
|---|---|---|
| **Sandbox** | An isolated environment where code or agents run without touching the rest of your system. | A playpen for robots. |
| **SDK** | Software development kit: a library that makes an API easy to use from a programming language. | A toolkit that makes the back door easy to use. |
| **Semantic search** | Searching by meaning (with embeddings), not just exact words. | Finding things that *mean* the same. |
| **Server tool** | A tool the AI provider runs for you (web search, web fetch, code execution). | A tool the AI company operates, so you don't have to. |
| **Serverless function** | Code that runs on demand in the cloud, with no server to manage. | A helper that only wakes up when called. |
| **Skill** | A folder of instructions (and optional scripts) an agent loads on demand for a specific task. | An instruction booklet the AI pulls out when needed. |
| **Slash command** | A saved prompt you trigger by typing `/name`. | A shortcut button for a long message. |
| **SOP** | Standard operating procedure: step-by-step instructions for a recurring task. | The recipe for doing a job the same way every time. |
| **Spend limit** | A hard cap on API spending, set in the provider's console. | A piggy bank limit so you never overspend. |
| **SSE** | Server-sent events: a way for servers to stream updates to clients. Older MCP remote transport. | A one-way stream of messages from a server. |
| **STAR method** | Situation, task, action, result: a structure for interview stories. | A simple way to tell a "what I did" story. |
| **stdio / Streamable HTTP** | The two MCP transports: local (the app launches the server) and remote (connect over a URL). | Plugging in at home vs. over the internet. |
| **Stop reason** | Why the model stopped: finished, hit max tokens, wants a tool, paused, refused. | Why the AI stopped talking. |
| **Streaming** | Receiving the model's output word by word as it's generated. | Watching the answer being typed. |
| **Structured output** | Forcing a model's reply to match a schema (e.g. valid JSON with specific fields). | Making the AI fill in a form instead of writing a paragraph. |
| **STT / TTS** | Speech-to-text (transcription) and text-to-speech (voice generation). | Ears and a mouth for computers. |
| **Subagent** | A helper agent with its own context, which a main agent delegates tasks to. | A helper robot sent on an errand. |
| **System prompt** | Standing instructions that set the model's role, rules and style for a conversation. | The AI's job description. |

## 🇹 T

| Term | Meaning | 🧸 ELI5 |
|---|---|---|
| **Tailscale** | A private network (VPN) that connects your devices securely without opening ports. | A secret tunnel between your devices. |
| **Temperature** | A randomness setting. Lower is more predictable, higher is more creative. | How wild or careful the AI's ideas are. |
| **TF-IDF** | A classic way to turn text into vectors by weighting rare, meaningful words. Used in the RAG kit. | Counting important words to find matching notes. |
| **Token** | A chunk of text (about ¾ of a word) that models read, write and bill by. | The little word-pieces AI reads and counts. |
| **Tool annotations** | Hints in MCP tool definitions (read-only, destructive, etc.) that help clients decide when to ask for approval. | Labels saying "safe" or "careful!" on each button. |
| **Tool poisoning** | Hiding malicious instructions in an MCP tool's description or output. | A trick button with secret bad instructions. |
| **Tool runner** | An SDK helper that runs the agent loop (call model → run tools → repeat) for you. | A helper that runs the robot loop for you. |
| **Tracing** | Recording every step an agent took (inputs, outputs, timing, cost) for debugging. | A video replay of what the robot did. |
| **Trigger** | The event that starts an automation (a schedule, webhook or new email). | The doorbell that starts the robot. |

## 🇺 U · 🇻 V

| Term | Meaning | 🧸 ELI5 |
|---|---|---|
| **Unified memory** | A design (e.g. Apple Silicon) where CPU and GPU share one big memory pool, great for local AI. | One big shared desk for all the computer's workers. |
| **VAD** | Voice activity detection: noticing when someone starts and stops speaking. | Knowing when you're talking. |
| **Vector** | A list of numbers, such as an embedding. | An address made of numbers. |
| **Vector database** | A database built to find embeddings that are "near" each other, i.e. similar in meaning. | A super-fast "who's standing nearby?" finder. |
| **Vibe coding** | Building software by describing what you want and letting AI write the code. | Telling the robot what app to build. |
| **Vision model** | A model that understands images. | AI with eyes. |
| **Voice banking** | Recording your voice so AI can create a personal synthetic voice, e.g. before losing speech. | Saving your voice so you can keep using it. |
| **VRAM** | Video RAM: the memory on a graphics card. It decides which models fit on a GPU. | The graphics card's own desk. |

## 🇼 W · 🇽 X · 🇾 Y · 🇿 Z

| Term | Meaning | 🧸 ELI5 |
|---|---|---|
| **WCAG** | Web Content Accessibility Guidelines: the standard for making websites usable by everyone. | The rule book for websites everyone can use. |
| **Webhook** | A URL that runs something when data is POSTed to it. It's how apps poke automations. | A doorbell URL that starts a robot. |
| **Workflow** | A sequence of automated steps, often visual (n8n, Zapier, Make). | A recipe the robot follows. |
| **Worktree (Git)** | An extra working folder of the same repo on its own branch, great for parallel agents. | A second desk for the same project. |
| **World model** | An AI that generates or simulates an interactive environment you can move through. | An AI that dreams up a world you can walk around in. |
| **Zero-shot** | Asking a model to do a task without giving examples. | Doing it without being shown how first. |

---

**Next:** [Appendix B · The Cheat Sheet →](b-cheat-sheet.md)
