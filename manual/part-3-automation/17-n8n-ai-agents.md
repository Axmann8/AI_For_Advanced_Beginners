...
The node runs the agent loop from [The Mental Model](../part-1-foundations/01-the-mental-model.md#-the-agent-loop-demystified):
...
| **Ollama** (local) | Free, private, unlimited, for simpler agents and high volume ([Local & Open Models](../part-7-local-ai/47-local-and-open-models.md)) |
...
| **Custom** (Data Tables, Sheets, vector stores) | Anywhere | Long-term facts ("my preferences"), see [Memory for Agents](../part-6-knowledge-and-memory/44-memory-for-agents.md) |
...
| **HTTP Request Tool** | Any API ([Webhooks, APIs & JSON](15-webhooks-apis-json.md)) |
| **Code Tool** | Custom logic in JavaScript or Python |
| **Call n8n Workflow Tool** | An *entire workflow* as one tool (see power move below) |
| **MCP Client Tool** | Every tool from any MCP server ([The Big MCP Server Catalog](../part-2-mcp-and-connectors/09-mcp-server-catalog.md)) |
...
Concepts explained in [RAG, Memory & Knowledge](../part-6-knowledge-and-memory/41-rag-memory-and-knowledge.md).
...
- Put approvals on anything that **sends, spends, deletes or posts publicly** ([MCP Security & Trust](../part-2-mcp-and-connectors/12-mcp-security-and-trust.md)).
...
- **Evaluations:** n8n's evaluation features run your workflow against a dataset of test inputs and score the results, so you
  can see if a prompt or model change helped ([Evaluating & Comparing AI](../part-10-mastery/74-evaluating-ai.md)).
...
[Build-Along: Your Pocket AI Assistant](../part-11-build-alongs/81-build-along-pocket-ai-assistant.md).
...
- **Pipelines:** fixed sequences like classify → extract → draft → review, each a small chain, which is often more reliable than
  one big agent.
- More patterns in [Multi-Agent Systems](../part-5-building-with-ai/39-multi-agent-systems.md).
...
- **Set API spend limits** in your provider console ([Cost Optimization](../part-10-mastery/75-cost-optimization.md)).
...
**Next:** [18 · Zapier & Make Walkthroughs →](18-zapier-and-make-walkthroughs.md)
